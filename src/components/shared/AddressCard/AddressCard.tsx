...
export default function AddressCard({
  ...
  return isCheckOut ? (
    <button
      type="button"
      onClick={onSelect}
      className={`w-full text-left p-4 border-2 ${isSelected ? "border-primary-500 bg-primary-50 ring-2 ring-primary-500/20" : "border-gray-200 hover:border-primary-200 hover:bg-gray-50"} rounded-xl transition-all duration-200 cursor-pointer`}
    >
      <div className="flex items-start gap-3">
        <div
          className={`w-10 h-10 flex items-center justify-center shrink-0 rounded-lg ${isSelected ? "bg-primary-500 text-white" : "bg-gray-100 text-gray-500"} transition-colors`}
        >
          {isSelected ? (
            <svg
              data-prefix="fas"
